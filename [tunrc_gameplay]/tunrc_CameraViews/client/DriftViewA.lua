...
local cameraOffset = Vector3(0, distance , 0)
...
local function update(deltaTime)
	...
	local targetCameraPosition = Vector3(math.sin(currentCameraRotation) * len, math.cos(currentCameraRotation) * len - (CameraHeightSpeedLimit() * 0.015), cameraHeight + (CameraHeightSpeedLimit() * 0.015))
	local targetLookOffset = lookAtOffset + Vector3(0, 0, 0)

	local targetCameraRoll = 0
	currentCameraRoll = 0

	local targetCameraFOV = 75 + math.sin(getVehicleSpeedString()) * deltaTime
