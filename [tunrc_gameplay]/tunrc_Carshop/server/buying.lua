...
addEvent("tunrc_Carshop.buyDonatVehicle", true)
addEventHandler("tunrc_Carshop.buyDonatVehicle", resourceRoot, function (model)
	if type(model) ~= "number" then
		triggerClientEvent(client, "tunrc_Carshop.buyVehicle", resourceRoot, false)
		return
	end
	...
	local level = client:getData("level")
	local donatmoney = client:getData("donatmoney")
	if not level or not donatmoney then
		triggerClientEvent(client, "tunrc_Carshop.buyVehicle", resourceRoot, false)
		return
	end
	...
	if priceInfo[4] > donatmoney then
		triggerClientEvent(client, "tunrc_Carshop.buyVehicle", resourceRoot, false)
		-- Недостаточно денег
		return
	end
	if exports.tunrc_Core:addPlayerVehicle(client, model) then
		client:setData("donatmoney", donatmoney - priceInfo[4])
		triggerClientEvent(client, "tunrc_Carshop.buyVehicle", resourceRoot, true)
	else
		triggerClientEvent(client, "tunrc_Carshop.buyVehicle", resourceRoot, false)
	end
