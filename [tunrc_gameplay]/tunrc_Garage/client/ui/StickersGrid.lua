...
function StickersGrid:getActiveItem()
...
function StickersGrid:draw(fadeProgress)
	...
	for i, item in ipairs(self.stickersList) do
		...
		local imageColor = tocolor(100, 100, 100, 255 * fadeProgress)
		local textColor = tocolor(100, 100, 100, 255 * fadeProgress)
		
		if itemX == self.selectionX and itemY == self.selectionY then
			if exports.tunrc_Config:getProperty("ui.dark_mode") == true then
				imageColor = tocolor(255, 255, 255, 255 * fadeProgress)
				textColor = tocolor(255, 255, 255, 255 * fadeProgress)
			else
				imageColor = tocolor(205, 205, 205, 255 * fadeProgress)
				textColor = tocolor(0, 0, 0, 255 * fadeProgress)
			end
		end
		local priceAlpha = 255
		if (item.price and money < item.price) or (item.level and level < item.level) then
			imageColor = tocolor(10, 10, 10, 50 * fadeProgress)
			priceAlpha = 150
		end
...
function StickersGrid:onKey(key)
	if self.isGridActive then
		...
		elseif key == "arrow_u" or key == "mouse_wheel_up" then
		...
		elseif key == "arrow_d" or key == "mouse_wheel_down" then
			self.selectionY = self.selectionY + 1
			if not self:getSelectedSticker() then
