		...
		if isElement(vehicle) then
			...
			if engine then
				...
				if getVehicleEngineState(vehicle) == true and distance < ENGINE_SOUND_DISTANCE*2 then
					...
					-- ALS
					if data.wasRevLimited and data.rpm < engine.maxRPM*0.98 then -- when using throttle
						...
						elseif data.prevThrottle and not data.throttle and data.rpm > engine.maxRPM*0.75 and math.random(1, 2) == 1 then 
							if data.als then 
								data.activeALS = true
							end
						elseif getKeyState("w") and getKeyState("space") and data.rpm > engine.maxRPM*0.95 then 
							if data.als then 
								data.activeALS = true
							end
						end
					...
					if not data.sounds then 
						...
						data.sounds[4] = playSound3D("sounds/"..soundPack.."/cutoff.wav", x, y, z, true)
						...
						if rpm < engine.maxRPM - 100 then
							vol = vol*0	
						elseif rpm > engine.maxRPM - 100 then
							vol = vol*ENGINE_VOLUME_MASTER*engine.volMult * 2
						end
						
						setSoundVolume(data.sounds[4], math.max(0, vol))
						setSoundSpeed(data.sounds[4], rpm/(engine.maxRPM*0.95))
