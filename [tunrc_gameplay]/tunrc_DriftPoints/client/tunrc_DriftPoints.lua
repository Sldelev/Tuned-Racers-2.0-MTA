...
local DRIFT_RESTRICT_TIME = 1

local MIN_COLLISION_FORCE = 1200
...
function finishCurrentDrift(restrictTime)
	...
	if driftPoints > 0 then
		...
		triggerEvent("tunrc_DriftPoints.earnedPoints", resourceRoot, driftPoints, pointsMultiplier, driftTimer)
	end
	if driftPoints > 0 then
...
local function update(dt)
	...
	if not isVehicleWheelOnGround(localPlayer.vehicle, 0) then
		return
	end
...
local function onCollision(hitElement, force)
	...
	if driftPoints > 501 then
		driftPoints = driftPoints - 500
	else
		driftPoints = 0
	end
	pointsMultiplier = pointsMultiplier - 1
