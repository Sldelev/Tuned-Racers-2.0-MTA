...
local function switchHandling()
	exports.tunrc_Vehicles:switchHandling()
end
	...
	return function(vehicle)
		if name == "handling" then
			if vehicle:getData("activeHandling") == "street" then
				return exports.tunrc_Lang:getString("context_menu_vehicle_enable_drift_mode")
			else
				return exports.tunrc_Lang:getString("context_menu_vehicle_disable_drift_mode")
			end
		elseif name == "engine" then
			if getVehicleEngineState(vehicle) then
				return exports.tunrc_Lang:getString("context_menu_vehicle_engine_off")
			else
				return exports.tunrc_Lang:getString("context_menu_vehicle_engine_on")
			end
		elseif name == "handbrake" then
			return exports.tunrc_Lang:getString("context_menu_vehicle_handbrake_on")
		elseif name == "lights" then
			if vehicle:getData("LightsState") then
				return exports.tunrc_Lang:getString("context_menu_vehicle_lights_off")
			else
				return exports.tunrc_Lang:getString("context_menu_vehicle_lights_on")
			end
		elseif name == "door" and type(value) == "number" then
			if vehicle:getDoorOpenRatio(value) > 0.5 then
				return exports.tunrc_Lang:getString(doorsLocales[value][1])
			else
				return exports.tunrc_Lang:getString(doorsLocales[value][2])
			end
