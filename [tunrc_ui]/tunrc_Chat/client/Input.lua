...
local function processCommand(inputText)
	...
	triggerEvent("tunrc_Chat.command", root, Chat.getActiveTab(), command, unpack(arguments))
	triggerServerEvent("tunrc_Chat.command", localPlayer, Chat.getActiveTab(), command, unpack(arguments))
	...
	if inputText ~= "" then
		if Chat.getActiveTab() then
			if utf8.sub(inputText, 1, 1) == "/" then
				...
				triggerEvent("tunrc_Chat.message", root, Chat.getActiveTab(), inputText)
				--triggerServerEvent("tunrc_Chat.message", root, Chat.getActiveTab(), inputText)
			end
