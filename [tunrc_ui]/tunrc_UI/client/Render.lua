...
function Render.start()
	...
	BLUR_ENABLED = exports.tunrc_Config:getProperty("ui.blur")
end
...
function Render.updateWidgetLocale(widget)
	if widget.locale then
		if widget.placeholder then
			widget.placeholder = exports.tunrc_Lang:getString(widget.locale)
		elseif widget.text then
			widget.text = exports.tunrc_Lang:getString(widget.locale)
		end
	end
end
...
addEvent("tunrc_Lang.languageChanged", false)
addEventHandler("tunrc_Lang.languageChanged", root, function (newLanguage)
	for resourceRoot, resourceInfo in pairs(Render.resources) do
