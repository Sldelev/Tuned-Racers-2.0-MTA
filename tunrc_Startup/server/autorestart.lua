...
local function showRestartWarning()
    ...
    if t then
        exports.tunrc_Chat:message(root, "global", "#FF0000* Перезапуск сервера через " .. tostring(t) .." минут")
...
function cancelRestart()
    if isTimer(restartTimer) then
        ...
        restartTimer = nil
        exports.tunrc_Chat:message(root, "global", "#FF0000* Перезапуск сервера отменён.")
