...
function changeVehicleOwner(owner, newOwner, vehicleId)
	if not isElement(owner) or not isElement(newOwner) then
		return false
	end
	if not vehicleId then
		return false
	end
	UserVehicles.changeOwner(vehicleId, newOwner:getData("_id"))
	updatePlayerVehiclesCount(owner)
	updatePlayerVehiclesCount(newOwner)
end
