...
function Users.registerPlayer(player, username, password, callback)
    ...
    exports.tunrc_Logger:log("auth", string.format("New user registered: '%s'", username))
    return true
end
...
function Users.loginPlayer(player, username, password, callback)
    ...
    return DatabaseTable.select(USERS_TABLE_NAME, {}, { username = username }, function(result)
        ...
        if success then
            ...
            if success then
                exports.tunrc_Logger:log("auth", string.format("User '%s' logged in with nickname '%s'", username, tostring(player.name)))
...
function Users.updateUserPassword(username, password, callback)
    ...
    -- Хэширование пароля
    password = hashUserPassword(username, password)
    exports.tunrc_Logger:log("auth", string.format("User '%s' changed password", username))
    -- Обновление пароля
...
function Users.logoutPlayer(player, callback)
    ...
    exports.tunrc_Logger:log("auth", string.format("User '%s' has logged out (%s)", username, tostring(player.name)))
...
addEvent("tunrc_Core.registerRequest", true)
addEventHandler("tunrc_Core.registerRequest", resourceRoot, function(username, password, betaKey)
    local player = client

    -- Проверка ключа
    if BETA_KEY_CHECK_ENABLED then
        if not BetaKeys.isKeyValid(betaKey) then
            triggerClientEvent(player, "tunrc_Core.registerResponse", resourceRoot, false, "beta_key_invalid")
            triggerEvent("tunrc_Core.register", player, false, "beta_key_invalid")
            return
        end
    end
    ...
    local success, errorType = Users.registerPlayer(player, username, password, function(result)
        ...
        triggerClientEvent(player, "tunrc_Core.registerResponse", resourceRoot, result, errorType)
        triggerEvent("tunrc_Core.register", player, result, errorType)
    end)
    if not success then
        triggerClientEvent(player, "tunrc_Core.registerResponse", resourceRoot, false)
        triggerEvent("tunrc_Core.register", player, false, errorType)
    end
end)

addEvent("tunrc_Core.loginRequest", true)
addEventHandler("tunrc_Core.loginRequest", resourceRoot, function(username, password)
    local player = client
    local success, errorType = Users.loginPlayer(player, username, password, function(result, errorType)
        triggerClientEvent(player, "tunrc_Core.loginResponse", resourceRoot, result, errorType)
        triggerEvent("tunrc_Core.login", player, result, errorType)
    end)
    if not success then
        triggerClientEvent(player, "tunrc_Core.loginResponse", resourceRoot, false, errorType)
        triggerEvent("tunrc_Core.login", player, false, errorType)
    end
end)

addEvent("tunrc_Core.logoutRequest", true)
addEventHandler("tunrc_Core.logoutRequest", resourceRoot, function(username, password)
    local player = client
    local success = Users.logoutPlayer(player, function(result)
        triggerClientEvent(player, "tunrc_Core.logoutResponse", resourceRoot, result)
        triggerEvent("tunrc_Core.logout", player, result)
    end)
    if not success then
        triggerClientEvent(player, "tunrc_Core.logoutResponse", resourceRoot, false)
        triggerEvent("tunrc_Core.logout", player, false)
    end
end)

addEvent("tunrc_Core.passwordChangeRequest", true)
addEventHandler("tunrc_Core.passwordChangeRequest", resourceRoot, function (password)
    local player = client

    if not Users.isPlayerLoggedIn(player) then
        triggerClientEvent(player, "tunrc_Core.passwordChangeResponse", resourceRoot, false)
        return false
    end
    local username = player:getData("username")
    local success = Users.updateUserPassword(username, password, function (result)
        triggerClientEvent(player, "tunrc_Core.passwordChangeResponse", resourceRoot, result)
    end)
    if not success then
        triggerClientEvent(player, "tunrc_Core.passwordChangeResponse", resourceRoot, false)
    end
