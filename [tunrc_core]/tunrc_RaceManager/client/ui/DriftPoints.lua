...
local function draw()
	...
	dxDrawTextShadow(exports.tunrc_Lang:getString("race_drift_score") .. ": " .. tostring(drawScore), 0, 0, screenSize.x - borderOffset, screenSize.y, tocolor(255, 255, 255), 1, font, "right", "top", 0)
	local y = 60
...
local function updateScoreList()
    ...
    for i, player in ipairs(players) do
    	if players[i + 1] and players[i + 1] == localPlayer then
    		isFirst = false
    		table.insert(playersScoreList, {i, exports.tunrc_Utils:removeHexFromString(player.name), player:getData("raceDriftScore")})
    	elseif players[i] == localPlayer then
    		table.insert(playersScoreList, {i, exports.tunrc_Utils:removeHexFromString(player.name), player:getData("raceDriftScore"), true})
...
function DriftPoints.start()
	...
	listFont = exports.tunrc_Assets:createFont("Roboto-Regular.ttf", 16, true)
	isActive = true
	...
	addEventHandler("tunrc_DriftPoints.earnedPoints", root, earnedPoints)
	scoreListUpdateTimer = setTimer(updateScoreList, 1000, 0)

	themeColor = tocolor(exports.tunrc_UI:getThemeColor())
end
...
function DriftPoints.stop()
	...
	removeEventHandler("tunrc_DriftPoints.earnedPoints", root, earnedPoints)
