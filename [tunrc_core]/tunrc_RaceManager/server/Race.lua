...
function Race:init(settings, map)
	...
	self.state = ""
	self.settings 	= exports.tunrc_Utils:extendTable(settings, RACE_SETTINGS)
	self.map 		= exports.tunrc_Utils:extendTable(map, RACE_MAP)
...
function Race:destroy()
	...
	triggerEvent("tunrc_RaceManager.raceDestroyed", self.element)
...
function Race:addPlayer(player)
	...
	if exports.tunrc_Utils:getVehicleOccupantsCount(player.vehicle) > 1 then
