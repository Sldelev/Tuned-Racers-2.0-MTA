...
local MOUSE_LOOK_DISTANCE_MIN = 2
...
local function update(deltaTime)
	...
	for k, v in pairs(targetCamera) do
		if k == "rotationHorizontal" then
			local diff = exports.tunrc_Utils:differenceBetweenAngles(camera[k], targetCamera[k])
	...
	local shakeOffset
	if currentCameraState == "freeLookCamera" or currentCameraState:find("side") == 1 then
		shakeOffset = Vector3(0, 0, 0)
	else
		...
		shakeOffset = Vector3(shakeX, shakeY, shakeZ)
	end

	local pitch = math.rad(camera.rotationVertical)
	local yaw = math.rad(camera.rotationHorizontal + 20)
	...
	Camera.setMatrix(
		camera.targetPosition + cameraOffset * camera.distance / 1.15,
		camera.targetPosition - shakeOffset * 12,
		camera.roll,
		camera.FOV
	)

	--outputDebugString("currentCameraState: " .. tostring(currentCameraState))
	camera.rotationHorizontal = exports.tunrc_Utils:wrapAngle(camera.rotationHorizontal)
end
...
function CameraManager.setState(name, noAnimation, animationSpeed)
	...
	for k, v in pairs(cameraPresets[name]) do
		if k == "targetPosition" then
			...
			if k == "rotationHorizontal" then
				v = exports.tunrc_Utils:wrapAngle(v)
			end
			targetCamera[k] = v
			if noAnimation then
				camera[k] = v
...
function CameraManager.setMouseLookEnabled(state)
