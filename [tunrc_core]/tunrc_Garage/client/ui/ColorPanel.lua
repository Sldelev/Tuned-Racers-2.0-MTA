...
function ColorPanel:draw(fadeProgress)
	...
	for i, bar in ipairs(self.bars) do
		...
		-- Подпись
		dxDrawText(bar.text, self.x + self.barOffset, y, self.x + self.resolution.x - self.barOffset, y + self.labelHeight, tocolor(255, 255, 255, a * fadeProgress), 1, Assets.fonts.menuLabel, "left", "center")

		-- Значение
		dxDrawText(("%d"):format(bar.value * 255), self.x + self.barOffset, y, self.x + self.resolution.x - self.barOffset, y + self.labelHeight, tocolor(255, 255, 255, a * fadeProgress), 1, Assets.fonts.menuLabel, "right", "center")
