...
function Language.set(lang)
	...
	triggerEvent("tunrc_Lang.languageChanged", root, currentLanguage)
...
addEventHandler("onClientResourceStart", resourceRoot, function ()
	...
	local language = exports.tunrc_Config:getProperty("ui.language")

	if not Locales.isValid(language) then
		exports.tunrc_Config:setProperty("ui.language", Language.getLocalization())
		return
	end
	...
	if not language then
		...
		exports.tunrc_Config:setProperty("ui.language", language)
	else
...
-- TODO: убрать
addEvent("tunrc_Lang.chatMessageServer", true)
addEventHandler("tunrc_Lang.chatMessageServer", resourceRoot, function (name, ...)
